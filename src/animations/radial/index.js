...
export function init(root = document) {
  ...
  const SVG = section.querySelector('#radial_svg');
  ...
  const CONFIG = {
    canvasSize: SIZE,
    tiny: { radius: 290, length: 2, density: 1.4, opacity: 0.5 },
    solid: { radius: 300, opacity: 0.2 },
    micro1: { radius: 330, length: 6, density: 1.4, opacity: 0.3 },
    micro2: { radius: 360, length: 16, density: 0.7, opacity: 0.25 },
  ...
  function ringByDensity({ radius, length, density, opacity, thickness, color }) {
    ...
    for (let i = 0; i < total; i++) {
      ...
      g.appendChild(
        polarLine(radius, length, deg, thickness ?? THEME.baseThickness, opacity, color ?? THEME.color)
      );
    }
    return g;
  }
  ...
  function addMarker({ atDeg, radius, base, height, color, opacity, radialOffset = 0 }) {
    const a = ((atDeg - 90) * Math.PI) / 180,
      rC = radius + radialOffset;
    const cx = CX + rC * Math.cos(a),
      cy = CY + rC * Math.sin(a);
    const tx = -Math.sin(a),
      ty = Math.cos(a);
    const rx = Math.cos(a),
  ...
  rotor.setAttribute('id', 'rotor');
  SVG.appendChild(rootGroup);

  rootGroup.appendChild(
    ringByDensity({ ...CONFIG.tiny, thickness: THEME.baseThickness, color: THEME.color })
  );
  if (CONFIG.marker?.enabled) {
    rootGroup.appendChild(addMarker({ ...CONFIG.marker, color: THEME.color }));
  }
  rootGroup.appendChild(
    circle(CONFIG.solid.radius, THEME.baseThickness * THEME.solidMultiplier, CONFIG.solid.opacity, THEME.color)
  );
  rotor.appendChild(
    ringByDensity({ ...CONFIG.micro1, thickness: THEME.baseThickness, color: THEME.color })
  );
  rotor.appendChild(
    ringByDensity({ ...CONFIG.micro2, thickness: THEME.baseThickness, color: THEME.color })
  );
  rotor.appendChild(
    ringByCount({ ...CONFIG.major, thickness: THEME.baseThickness, color: THEME.color })
  );
  rootGroup.appendChild(rotor);
  ...
  function playIntro(force = false) {
    ...
    const step = (now) => {
      ...
      const a = start + (end - start) * INTRO_EASE(t);
      angle = a;
      applyAngle(angle);
  ...
  }, { threshold: [OBSERVE_RATIO], root: null });
  ...
  function up(ev) {
    ...
    const v0 = Math.max(-MAX_VELOCITY, Math.min(MAX_VELOCITY, velocity)) * INERTIA_GAIN;
  ...
  return () => {
    ...
    SVG.removeEventListener('dragstart', preventDrag);
